...
describe('Independent fence takeoff regressions', () => {
  for (const slug of ['fence-calculator','fence-post-calculator','fence-panel-calculator']) {
    it(`${slug}: exact divisions, last short section and spares`, () => {
      const exact = calculate(slug, {length:80, spacing:8, waste:0});
      expect(value(exact,'posts')).toBe(11);
      expect(value(exact,'panels')).toBe(10);
      const remainder = calculate(slug, {length:81, spacing:8, waste:10, extraPosts:2});
    ...
    it(`${slug}: prices panels, not posts; per-foot quote does not buy spare feet`, () => {
      const rows = calculate(slug,{price:10});
      expect(value(rows,'cost')).toBe(150);
      expect(rows.find(r=>r.key==='cost')!.label).toBe('Estimated panel material cost');
      expect(value(calculate(slug,{price:10},{price:'USD/ft'}),'cost')).toBe(1000);
      expect(value(calculate(slug,{price:0}),'cost')).toBe(0);
      expect(calculate(slug).some(r=>r.key==='cost')).toBe(false);
    });
    it(`${slug}: metric equivalents preserve counts at an exact boundary`, () => {
      const rows=calculate(slug,{length:24.384,spacing:2.4384,waste:0},{length:'m',spacing:'m'});
      expect(value(rows,'posts')).toBe(11);
      expect(value(rows,'panels')).toBe(10);
    });
  }
