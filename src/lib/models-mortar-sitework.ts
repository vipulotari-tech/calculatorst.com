...
function weightModel(label:string,density:number):Model{
  return {
    ...
    sources:[inchGravel],
    calculate(v){const mode=Math.round(v.mode),ft3=mode===0?v.length*v.width*v.depth:mode===1?v.area*v.depth:v.volume,yd3=ft3/27,orderYd3=yd3*waste(v),tons=orderYd3*v.density,lb=tons*2000;return result([row('tons',`Estimated ${label.toLowerCase()} order weight`,tons,'US tons'),row('pounds','Estimated order weight',lb,'lb'),row('kg','Estimated order weight',lb/LB_PER_KG,'kg'),row('tonnes','Estimated order weight',lb/LB_PER_KG/1000,'metric tonnes'),row('net','Measured volume',yd3,'yd³'),row('order','Volume with allowance',orderYd3,'yd³')],[`${fmt(ft3)} ft³ ÷ 27 = ${fmt(yd3)} yd³.`,`${fmt(orderYd3)} yd³ × ${fmt(v.density)} = ${fmt(tons)} US tons.`]);}
  };
}
...
const excavationGeneral:Model={
  fields:[length('length','Bottom excavation length',20),length('width','Bottom excavation width',10),length('depth','Excavation depth',4),positiveOrZero(number('sideSlope','Side slope — horizontal per 1 vertical',0,0,'Enter 0 for vertical sides.')),number('swell','Bank-to-loose swell (%)',20,0),price('USD/yd3')],
...
const trenchGeneral:Model={
  fields:[length('length','Trench length',100),length('bottomWidth','Bottom trench width',2),length('depth','Trench depth',4),positiveOrZero(number('sideSlope','Side slope — horizontal per 1 vertical',0,0)),number('swell','Bank-to-loose swell (%)',20,0),price('USD/yd3')],
...
const dirtRemoval:Model={
  fields:[length('length','Excavation length',20),length('width','Excavation width',10),length('depth','Excavation depth',4),number('swell','Bank-to-loose swell (%)',20,0),positiveOrZero(number('truckCapacity','Loose truck capacity (yd³)',12,0)),number('haulRate','Haul / disposal rate (USD / loose yd³)',0,0),number('tripFee','Fixed fee per truck trip (USD)',0,0)],
