...
const gravelDepth:Model={
  ...
  sources:[omniGravel],
  calculate(v){const a=Math.round(v.areaMode)===0?v.length*v.width:v.area;requireCondition(a>0,'area','Coverage area must be greater than zero.');const weightMode=Math.round(v.supplyMode)===1;if(weightMode) requireCondition(v.density>0,'density','Bulk density must be greater than zero when calculating depth from weight.');const yd3=weightMode?v.tons/v.density:v.volume/27;const ft3=yd3*27,depth=ft3/a;return result([row('depth','Average coverage depth — inches',depth*12,'in'),row('mm','Average coverage depth — millimeters',depth/FT_PER_M*1000,'mm'),row('area','Coverage area',a,'ft²'),row('volume','Available gravel volume',yd3,'yd³')],[`${fmt(yd3)} yd³ × 27 = ${fmt(ft3)} ft³.`,`${fmt(ft3)} ft³ ÷ ${fmt(a)} ft² = ${fmt(depth*12)} in average depth.`]);}
};
